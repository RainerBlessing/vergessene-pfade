...
- Umsetzung:
  ...
  - **Nachtrag (2026-09-19), Review von Stage 4 und Nacharbeiten:** Sieben Befunde behoben. Wichtig: Der Abspann verschluckte die Inschrift über den Besucher, wenn man erst übernachtete (jetzt wird sie gelesen, dann folgt die Szene); zwei „Verluste“ des Kompromisses änderten gar nichts, weil dort schon Stümpfe standen (neue Felder, und der Test verlangt jetzt eine echte Änderung gegenüber der Karte); der Bau mit Jungfüchsen hatte keinen Untersuchungstext; Escape auf dem Futon öffnete trotzdem die Frage; `OBS_SETTLED` war totes Wissen und ist entfernt; die Ortsanzeige wurde beim Kartenwechsel nicht neu bestimmt (samt Datentest, dass kein Übergang in einem Raum liegt); der Totholzstapel im Wald stand auf Kies und hat jetzt ein eigenes Tile.
  - **Fenster und Vollbild (2026-09-22, auf Wunsch von Rainer):** `--scale 2..5` und `--fullscreen` beim Start, im Spiel F4 (Größe) und F11 (Vollbild); das Bild bleibt ganzzahlig skaliert. **Befund beim Ausmessen:** Unter Hyprland kachelt der Fenstermanager das Fenster (gemessen 518x850 statt 640x400), die Größenwahl bleibt dort wirkungslos. Im README steht die passende Fensterregel.
  - **Review der Klangarbeit (2026-09-22):** Zehn Befunde, alle behoben. Kritisch: `audio_close` lief über einen uninitialisierten Zustand, weil jeder `goto cleanup` die Initialisierung übersprang. Schwer: Schritte als Ereignisse füllten den 32er-Ringpuffer, sodass echte Ereignisse (Beobachtungen, Begegnungen) verworfen wurden und im Protokoll abgeschnittene Zeilen standen; Schritte laufen jetzt über einen Zähler und tauchen im Log gar nicht mehr auf. Weiter: Escape sprang doch noch in den Abspann; der Pegel 2,2 übersteuerte einzelne Klänge (jetzt 1,35, mit Test gegen Übersteuern je Klang); Hüllkurven endeten mit hörbarem Knacks; die Label-Prüfung war wegen „KLANG“ immer wahr; fehlendes Audiogerät verhinderte den Start; ~100 ms Verzögerung auf ~40 ms gesenkt; zwei versehentlich committete Screenshots (5,9 MB) entfernt und `.gitignore` erweitert.
  - **Startseite (2026-09-19, auf Wunsch von Rainer):** Eine überspringbare Seite vor der Ankunftsszene nennt Titel, Ziel des Spiels und die vier Tasten, dazu den Hinweis, sich zum Untersuchen vor ein Objekt zu stellen. Eigener Zustand `GAME_TITLE`; nur Enter geht weiter, Laufen und Tasche bleiben gesperrt. Der Text liegt als Datentabelle `title_page` vor, ein Test prüft Zeilenbreite und dass die Tasten genannt sind.
  - **Lautstärke angehoben (2026-09-19, auf Wunsch von Rainer):** Gesamtpegel auf etwa das Doppelte, „leise“ entspricht dem bisherigen Normalpegel. Die Summe wird beim Mischen begrenzt, damit gleichzeitige Klänge nicht übersteuern; ein Test prüft beides.
  - **Schrittgeräusch (2026-09-19, auf Wunsch von Rainer nachträglich ergänzt):** zwei abwechselnde, sehr leise Tritte beim tatsächlichen Gehen (Ereignis `EV_STEP`, nicht im Log). Ein Test hält fest, dass sie leiser sind als das Bestätigungs-Klacken.
  - Klang, dritte Fassung (2026-09-19): **kurze Effekte an einzelnen Handlungen** statt einer Fläche, erzeugt in `src/audio.c` (Klacken, Schreiben, Schaben und Einrasten des Steins, Verband und Tierlaut, Keramik, zwei Holzschläge, Knarren beim Erscheinen, Treffer, Zerbrechen). Lautstärke je Effekt abgestimmt, gemeinsame Einstellung mit F3 (an/leise/aus), sichtbar im Notizbuch. Keine Schritte, keine Tipp-Geräusche, kein Dauerton. Eigener Test `sounds`: jeder Effekt hat einen hörbaren Körper und ist kürzer als eine Sekunde, jedes Ereignis trifft den richtigen Effekt, stille Ereignisse bleiben stumm, „aus“ schaltet wirklich ab.
  - Klang, zweite Fassung: **entfernt (2026-09-19, auf Wunsch von Rainer).** Die zweistimmige Fläche war beim Start als Summton hörbar und störte. Das Spiel läuft jetzt ohne Ton; das Konzept-Kriterium „Musik ändert sich“ bleibt damit offen und wäre mit echtem Material statt erzeugter Töne erneut anzugehen.
...
- **Nachtrag (2026-09-19), Fuchs nach dem Kampf, zweite Fassung:** Rund um den Bau stehen am Morgen frische Stümpfe; der leere Bau nennt Holzspäne und die weiterarbeitenden Holzfäller. Mio hat zwei Morgenzeilen: eine, die die Hilfe der Spielfigur anerkennt, und eine ohne. Der Ausgang überschreibt den versorgten Bau, damit Bild, Text und Dialog zusammenpassen; Fährten und Notizen bleiben erhalten.
- **Nachtrag (2026-09-19), Fuchs nach dem Kampf:** Der Bau war unmittelbar nach dem Sieg leer, wortlos und noch am selben Tag. Damit war das Versorgen des Fuchses (und die Fähigkeit „Tierzeichen“) für alle, die zuerst kämpfen, ohne Erklärung verloren. Jetzt bleibt der Fuchs am Tag der Entscheidung, zieht erst über Nacht fort, und der leere Bau lässt sich untersuchen („Der Bau ist leer. Kein Fuchs, kein Warmes, keine frischen Spuren.“) samt Notizbucheintrag.
